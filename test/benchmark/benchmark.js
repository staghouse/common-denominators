...
suite
  ...
  .on("cycle", event => {
    ...
    if (results.length === tests[0].fns.length * tests.length && cycles === 0)
      log();
  ...
  .on("complete", () => {
    ...
    for (let i = 1; i < tests.length; i++) {
      ...
      data.map((d, index) => {
        ...
        const currentDiffComparable = !currentDiffBetter && !currentDiffWorse;
        const currentDiffMsg = `${parseInt((currentDiff / current) * 100)}%`;
